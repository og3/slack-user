...
// 1, #create-direct_userにワードが入力された時の処理
  $("#create-direct_user").on("keyup", function() {
   // 変数inputに、#create-direct_userに入力された値を取得し、代入する
    var input = $("#create-direct_user").val();
    // urlの部分を区切って配列にする。
...
// 3 ,入力された値に応じてユーザー情報の表示する処理
  function add_user(data){
    ...
    var html =
      '<div class="create-direct-user clearfix">'                                                     +
        '<p class="create-direct-user__name">'                                                        +
          data.user_name                                                                                +
        '</p>'                                                                                     +
        '<a class="user-search-add create-direct-user__btn create-direct-user__btn--add" data-user-id="' + data.id + '" data-user-name="' + data.user_name + '">追加'                                                                                         +
        '</a>'                                                                                     +
      '</div>';
...
// 4,「追加ボタン」が押されたユーザー情報を取得する処理
    function get_user(){
      ...
      $(".create-direct-user__btn--add").on('click', function(e){
        e.preventDefault();
...
// 5, 4から送られたユーザーをグループに追加する処理
    function add_user_to_group(id, name){
      ...
      var appended_user_html =
       '<div class="create-direct-user  clearfix">'                                           +
        '<input name="direct[user_ids][]" type="hidden"' + 'value=' + id + '>'   +
        '<p class="create-direct-user__name">'                                     +
          name                                                                  +
        '</p>'                                                                  +
        '<a class="user-search-remove ' + 'create-direct-user__btn create-direct-user__btn--remove"' + 'data-user-id=' + id + '>削除'                                          +
        '</a>'                                                                  +
      '</div>';
      // #create-direct-usersにappended_user_htmlを追加
      $("#create-direct-users").append(appended_user_html);
    }

// 6, 5で追加されたユーザーを削除する
    $(function delete_user(){
      $("#create-direct-users").on('click','.create-direct-user',function(){
        $(this).remove();
      })
    }); 
  });
